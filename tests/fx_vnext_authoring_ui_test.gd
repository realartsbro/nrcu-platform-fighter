...
func _slider_drag_is_one_transaction(fx_id: String) -> void:
	...
	_check(shell.recipe_rows != null and shell.recipe_rows.get_child_count() >= 5 and shell.production_tab.is_ancestor_of(shell.library_rows) and shell.recipes_tab.is_ancestor_of(shell.recipe_rows), "UI-18 Hero Recipe library is separate from Production inventory")
