...
3. `edge_source_mode`: 0 = alpha, 1 = luma, 2 = both (max). There is NO
   ...
   Verdict vs handoff binding #11 (Side Field organic edge): rejection stands.
   #11 demands REAL contour semantics (silhouette-normal/distance), not a
   texture slot; the procedural alpha/luma contour path stays live, the
   influence gate is contour-aware (scanned SDF bands, not rect erosion),
   and the true normal-field primitive is reserved Phase 7 hero scope
   ("Side Field Organic Edge"). No creative capability is removed — only a
   dead slot that could never render.
